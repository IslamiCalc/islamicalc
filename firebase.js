// ============================================================
// IslamiCalc — firebase.js v6.0
// ✅ type="module" — لا window aliases مكشوفة
// ✅ islamiCalcReady يُطلق دائماً
// ✅ XSS fix — textContent فقط
// ✅ Rate Limiting — منع addXP المكرر
// ✅ Streak timezone fix
// ✅ Toast آمن — clearTimeout
// ✅ showUserMenu — لا memory leak
// ✅ Offline fallback — XP يُخزَّن محلياً
// ✅ getUserCountry — AbortController timeout
// ✅ _xpCooldowns cleanup
// ✅ logActivity sanitize
// ✅ window._islamiDB محذوف
// ============================================================
...
// ⚠️ فعّل App Check لمنع الاستخدام غير المصرّح
// ============================================================
...
// Rate Limiting لـ XP — مع cleanup تلقائي
const _xpCooldowns = {};
function _setCooldown(reason) {
  _xpCooldowns[reason] = Date.now();
  setTimeout(() => delete _xpCooldowns[reason], 10000);
}
...
function showToast(msg, type = "") {
  ...
  toast.textContent = msg;  // ✅ textContent فقط — لا innerHTML
...
async function loginWithGoogle() {
  try {
    ...
    const name   = (result.user.displayName || "أهلاً")
                     .split(" ")[0].substring(0, 20);
...
async function logout() {
  try {
    ...
    showToast("❌ تعذّر تسجيل الخروج", "toast-error");
  }
}

// ============================================================
// getUserCountry — ✅ AbortController بـ 3 ثوانٍ
// ============================================================
async function getUserCountry() {
  try {
    ...
    return typeof data.country === "string" ? data.country.substring(0, 5) : "unknown";
  } catch {
    return "unknown";
  }
}
...
// ============================================================
// رفع XP المخزّن محلياً — ✅ حد آمن 1000
// ============================================================
async function _flushLocalXP(uid) {
  ...
  if (stored > 0 && stored <= 1000) {
    try {
      ...
      // silent fail — سيحاول مرة أخرى عند الدخول التالي
    }
  } else if (stored > 1000) {
    // قيمة مشبوهة — نمسحها
    localStorage.setItem("userXP", "0");
...
async function ensureUserDoc(user) {
  try {
    ...
    if (!snap.exists()) {
      ...
      const newDoc  = {
        uid:          user.uid,
        name:         (user.displayName || "مجهول").substring(0, 40),
        email:        user.email || "",
        photo:        user.photoURL || "",
        xp:           0,
        level:        1,
...
// ============================================================
// Streak — ✅ إصلاح timezone + null safety
// ============================================================
...
// ============================================================
// addXP — ✅ Rate Limiting + Validation + Offline fallback
// ============================================================
...
// ============================================================
// safeAddXP / safeAddBadge
// ============================================================
function safeAddXP(amount, reason) {
  if (window.islamiCalc?.addXP) {
...
// ============================================================
// Activity Log — ✅ sanitize النصوص
// ============================================================
function logActivity(text, sub = "", icon = "⭐", color = "rgba(45,106,79,0.1)", xp = 0) {
  try {
    const safeText = String(text).substring(0, 80);
    const safeSub  = String(sub).substring(0, 80);
    ...
    activity.push({
      ...
      sub:   safeSub,
      icon:  safeIcon,
      color: "rgba(45,106,79,0.1)", // ثابت — لا نثق بالـ color القادم من الخارج
      xp:    Number.isInteger(xp) ? xp : 0,
  ...
  if (user) {
    if (user.photoURL) {
      ...
      img.onerror = () => {
        ...
        fb.textContent    = (user.displayName || "م")[0];
        fb.style.cssText  = "width:28px;height:28px;border-radius:50%;background:var(--primary);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-weight:900;margin-left:6px;font-size:14px;";
        loginBtn.insertBefore(fb, loginBtn.firstChild);
      };
      loginBtn.appendChild(img);
    }
    const span       = document.createElement("span");
    span.textContent = (user.displayName || "حسابي")
                         .split(" ")[0].substring(0, 20) + " ▾";
...
// ============================================================
// User Menu Popup — ✅ لا memory leak
// ============================================================
function showUserMenu() {
  ...
  photoEl.onerror = () => {
    ...
    fb.textContent     = (currentUser?.displayName || "م")[0];
    fb.style.cssText   = "width:60px;height:60px;border-radius:50%;border:3px solid var(--gold);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;background:var(--primary);color:#fff;margin:0 auto 10px;";
    menu.insertBefore(fb, photoEl.nextSibling);
  };
...
window.islamiCalc = {
  ...
  // Firebase refs للصفحات الداخلية فقط
  auth, db, doc, updateDoc, increment, serverTimestamp, arrayUnion,
};

// ✅ showToast مكشوفة عالمياً فقط (مطلوبة في كل الصفحات)
